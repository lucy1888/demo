...
if [ -z "$targets" ]; then
  ...
  $HDC_BIN -t "$target_id" shell aa force-stop cn.com.hzb.mobileban.per.kuiklydemo
  $HDC_BIN -t "$target_id" install entry/build/default/outputs/default/entry-default-signed.hap
  $HDC_BIN -t "$target_id" shell aa start -a EntryAbility -b cn.com.hzb.mobileban.per.kuiklydemo
  done
fi